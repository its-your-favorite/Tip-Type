 ...
 * raiseError
 * 
 * You can connect this function to your desired error-handler method
 */
...
TYPE.run = function(info) {
	...
	} else if (TYPE.getType(info[0]) === "Object") {/* Info a Numeric array & Not an assoc array? : make it one */
		for( x = 0; x < info.length; x++) {
	...
	for( x = 0; x < expected.length; x++) {/* For each actual parameter */
		...
		if(passed[x] !== null) {/* An actual parameter was provided already */
			var type = (info[x][type_key]);
			var okay = false, def = [];
			var value = passed[x];
			
			if( typeof (info[x][type_key]) !== null) {/* Typeof param was included */
				
				if (TYPE.getType(type) !== "Array") { /* always treat as array*/
					type = [type];
				}

				if (info[x].hasOwnProperty(default_key) ) {
					def = info[x][default_key];
				}
			...
			if(info[x][default_key] == TYPE.Err) {/* Not an optional parameter */
	...
	if((passed.join(",") != next.join(",")) && (!failed)) {
		...
		return TYPE.run.caller.apply(this, next);
	}
	return false;
};
